...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            f'{Path(__file__).with_suffix(".log")}', mode="a"
        ),  # Append to file
...
def process_openai_corpus(
    max_entries=1000, repeats: int = 3, service_tier: str = "flex"
):
    ...
    while i < (max_entries * repeats) and (exc_counter < max_exceptions):
        with get_session() as db:
            ...
            entries = (
                db.query(BenjaminEntry)
                .filter(BenjaminEntry.hash.is_not(None))
                .filter(~BenjaminEntry.hash.in_(excluded_hashes_query))
                .filter(BenjaminEntry.scrape_state < 10)
                .limit(1)
                .all()
            )
            ...
            for entry in entries:
                logging.info(
                    f"Corpus: Processing Entry {entry.id}... Exception Counter: {exc_counter}"
                )
                ...
                try:
                    for repeat in range(repeats):
                        ...
                        new_openai = OpenAIAnalyzationCorpus(
                            repeat=repeat, entry_id=entry.id, hash=entry.hash
                        )

                        response, result = classifier_openai.classify(
                            input_text=entry.text or entry.text_raw,
                    ...
                    if exc_counter > max_exceptions:
                        raise Exception(
                            f"Excepetion Counter exceeded {max_exceptions}!"
                        )
