...
in_filename = (
    "c:/temp/thesis/walter-benjamin-gesammelte-briefe-baende_1_bis_2_027-874.pdf"
)
out_filename = (
    "c:/temp/thesis/walter-benjamin-gesammelte-briefe-baende_1_bis_2_027-874.txt"
)
...
def parse_letters_from_pdf(pdf_path, store_to_database: bool = True):
    ...
    date_pattern = re.compile(r"^(\d+)(.*?)(\d+)$", re.MULTILINE)
...
def printout_letters(letters, out_filename):
    with open(out_filename, "wt", encoding="utf-8") as file:
        for letter in [l for l in letters if l.scrape_comment]:
            file.write(
                f"{letter.year}-{letter.month}-{letter.day}. Errors: {letter.scrape_comment}\n"
            )

        file.write(
            yaml.dump(receivers, default_flow_style=False, allow_unicode=True, indent=2)
        )
...
if __name__ == "__main__":
    ...
    if store_to_database:
        with get_session() as db:
            entries = (
                db.query(BenjaminEntry).filter(BenjaminEntry.scrape_state < 23).all()
            )
