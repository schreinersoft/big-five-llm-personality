...
def parse_letters_from_pdf(
    in_path, volume, start_page, end_page, store_to_database: bool = True
):
    """
...
def printout_letters(letters, out_filename):
    with open(out_filename, "wt", encoding="utf-8") as file:
        for letter in [l for l in letters if l.scrape_comment]:
            file.write(
                f"{letter.year}-{letter.month}-{letter.day}. Page: {letter.page}. Errors: {letter.scrape_comment}\n"
            )
...
if __name__ == "__main__":
    ...
    if scrape:
        letters = parse_letters_from_pdf(
            in_filename,
            volume=1,
            start_page=34,
            end_page=350,
    ...
    store_to_database = False  # !!!DONE
    in_filename = (
        "c:/Temp/thesis/The diary of Virginia Woolf Volume Three 1925-1930.pdf"
    )
    out_filename = (
        "c:/Temp/thesis/The diary of Virginia Woolf Volume Three 1925-1930.txt"
    )
    if scrape:
        letters = parse_letters_from_pdf(
            in_filename,
    ...
    out_filename = (
        "c:/Temp/thesis/The Diary of Virginia Woolf Volume Four 1931-1935.txt"
    )
    if scrape:
        letters = parse_letters_from_pdf(
            in_filename,
    ...
    out_filename = (
        "c:/Temp/thesis/The Diary of Virginia Woolf Volume Five 1936-1941.txt"
    )
    if scrape:
        letters = parse_letters_from_pdf(
            in_filename,
    ...
    if store_to_database:
        with get_session() as db:
            entries = db.query(WoolfEntry).filter(WoolfEntry.scrape_state < 3).all()
