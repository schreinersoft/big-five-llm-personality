        ...
        while current:
            ...
            if "letter-date" in class_names:
                try:
                    ...
                    entry.scrape_comment += (
                        f"{datetext} : Unparsable Date in {file_name}"
                    )
...
if __name__ == "__main__":
    ...
    if False:
        for file_name in os.listdir(in_dir):
            ...
            with open(file_path, "r", encoding="utf-8", errors="ignore") as file:
                data = file.read()
                result = scrape_multiple_letters(
                    data, store_to_database=True, file_name=file_name
                )
    ...
    if store_to_database:
        with get_session() as db:
            entries = db.query(OrwellEntry).filter(OrwellEntry.scrape_state < 3).all()
            ...
            for entry in entries:
                ...
                entry.author_age = int(
                    (
                        (
                            (entry.year * 12)
                            + entry.month
                            - ((birth_year * 12) + birth_month)
                        )
                    )
                    / 12
                )
