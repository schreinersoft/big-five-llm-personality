...
def process_google(
    ...
    with get_session() as db:
        while i < (max_num * repeats):
            ...
            for essay in essays:
                print(
                    f"V4: Processing Essay {essay.id} with temperature {temperature}..."
                )
                for repeat in range(repeats):
                    print(f"{repeat + 1}. Repeat")
                    try:
                        response, result = classifier_google.classify(
                        ...
                        new_openai = GoogleAnalyzation(
                            ...
                            model=response.get("measurement_version"),
                            temperature=temperature,
                            input_tokens=response["usage_metadata"][
                                "prompt_token_count"
                            ],
                            output_tokens=response["usage_metadata"][
                                "total_token_count"
                            ]
                        ...
                        new_openai = OpenAIAnalyzationV3(
                            essay_id=essay.id, error_response=str(e)
                        )
                        db.add(new_openai)
                    finally:
                        i += 1
